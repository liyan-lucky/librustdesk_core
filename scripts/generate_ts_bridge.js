...
const CORE_RS = path.join(projectRoot, 'rustdesk-master', 'src', 'harmony_bridge', 'core.rs');
const INDEX_D_TS = path.join(projectRoot, 'cpp', 'types', 'librustdesk_bridge', 'index.d.ts');
const NATIVE_BRIDGE_TS = ''; // HAP应用层文件，核心项目中不生成
const LIB_BRIDGE_D_TS = ''; // HAP应用层文件，核心项目中不生成

const coreContent = fs.readFileSync(CORE_RS, 'utf-8');
const indexContent = fs.readFileSync(INDEX_D_TS, 'utf-8');
const nativeContent = NATIVE_BRIDGE_TS ? fs.readFileSync(NATIVE_BRIDGE_TS, 'utf-8') : '';
const libBridgeContent = LIB_BRIDGE_D_TS ? fs.readFileSync(LIB_BRIDGE_D_TS, 'utf-8') : '';
