...
- Online run `27457899059` showed the GitHub SDK zip can omit SDK libc++ headers entirely. The workflow installs MSYS2 `mingw-w64-clang-x86_64-libc++`, and the script can fall back to `RUSTDESK_HARMONY_LIBCXX_INCLUDE`, `OHOS_LIBCXX_INCLUDE`, or `C:\msys64\clang64\include\c++\v1` while keeping OHOS sysroot for C/system headers.
...
- If the online SDK zip is a minimal `native/llvm + native/sysroot` package, do not assume SDK libc++ headers are present. Either set `RUSTDESK_HARMONY_LIBCXX_INCLUDE` or install MSYS2 clang64 libc++ headers and let the script discover `C:\msys64\clang64\include\c++\v1`.
