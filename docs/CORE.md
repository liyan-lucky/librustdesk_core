...
- 2026-06-15 源码更新（中文说明，已发布标签 `core-81`）：共享/被控链路新增 `captureRequired` 中间态和 OHOS `scrap` 入站帧源。`main_start_service(true)` 不再假装 ready，而是返回 `captureRequired=true`、`incomingReady=false`，让 App 启动 native screen capture 并推送首帧；`scrap::common::ohos::Capturer::frame()` 会从核心 incoming frame cache 取最新帧并返回 `Frame::PixelBuffer`，`Display::primary/all` 返回当前或兜底分辨率。旧 mirror 同步 `captureRequired` 和 frame cache 清理，避免复制回退。本地核心构建通过，产物 `128,894,588` bytes，SHA256 `2DC3B655664B756E255684D28FBA0CB3A9DEC14E6080EA4682FA26486ADF9B6D`；GitHub Actions run `27563925971` 已成功发布 `core-81`，线上 asset `131,631,706` bytes，SHA256 `64463fa57005cd5ccd99bafa9a40f18a9d605f8e90f5e199f92b38abfcdb4829`，release body 已补中文说明。11 App 使用本地同源核心构建 `0.22.6` 后验包、66 项连接链路审计、无线安装启动和干净 hilog 均通过。
...
- 2026-06-19 release 二次修复：run `27852266805` 两个 build job 成功后，release job 因产物检查后执行 `actions/checkout` 清理了 `./release-assets`，生成了空 `core-24` release；workflow 已移除该 checkout，并启用 `fail_on_unmatched_files: true`。空 `core-24` release/tag 已删除，run `27853110949` 已成功发布 `core-25` 双资产。
...
- `src/harmony_bridge/core.rs`
  ...
  - 入站共享采集帧必须通过独立 `incoming_screen_frame` 缓存进入核心；不能复用出站远控 `latest_video_frame`，也不能因为 native buffer 有帧就把 `incomingReady` 置 true。
  - `captureRequired=true` 只表示核心正在等待 App 录屏首帧，不能作为服务 ready 对外展示；`incomingReady=true` 必须等 desktop server/video source 真正消费帧并可对外服务。
