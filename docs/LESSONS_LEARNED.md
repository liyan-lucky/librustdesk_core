...
## 2026-06-22：workflow 绿色不等于包内 Core 正确

第一次 App Release run `27920529277` 虽然绿色，但旧隐藏 URL secret 让 arm64 混入历史 `A200A8...`，x86_64 则来自新 `E58746...`。该包已作废。App workflow 已移除 secret/var 的静默 URL 优先级，改为默认 latest 或通过 dispatch 显式固定两架构 URL+SHA256；最终 run `27920708116` 的 HAP 同时命中 `core-34` arm64 `90A283...` 与 x86_64 `E58746...`。以后必须下载 HAP 后扫描双架构 CoreBuildInfo、大小、哈希和 ABI，不能仅看 workflow、标签或版本号。
