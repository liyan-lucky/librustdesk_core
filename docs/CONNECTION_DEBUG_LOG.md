...
- 11 App 项目的 `entry/src/main/cpp` 已经把聊天发送修成四参调用，但 13 核心项目的 `cpp/rustdesk_bridge_abi.h` 和 `cpp/rustdesk_bridge_loader.cpp` 仍是旧一参调用。
- Rust `native_rust_core/src/bridge_api.rs` 的 `rustdesk_bridge_session_send_chat` 是四参 ABI，核心源项目不修会在下一次同步 `cpp/` 时覆盖 app 修复。
...
- Current CI fix: keep VP8/VP9 encoders enabled but build only `make libvpx.a`, then manually install `libvpx.a`, public `vpx/*.h` headers, and `vpx.pc`. This skips the unused `libvpxrc.a` target that pulls in `vp9/ratectrl_rtc.cc` and `vp8/vp8_ratectrl_rtc.cc`.
- Local validation for the current fix used `build_debug_20260613_074001.log`: `libvpx-build.log` had no `[CXX]`, no `ratectrl_rtc`, and no `libvpxrc`; full Cargo exited `0`; local core remained `129,592,014` bytes, SHA256 `32F3B3AC37EC82C94F2B4B3BA041459D2AF8ADA6AB1F3A57B39056F460F61B5F`.
