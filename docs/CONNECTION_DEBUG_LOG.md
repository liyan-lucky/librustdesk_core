...
- Online run `27457899059` for commit `ecb8a63` failed earlier in script validation: the secret-provided SDK zip had no `native/llvm/include/libcxx-ohos/include/c++/v1/cstdint` and no `native/llvm/include/c++/v1/cstdint`. The workflow now installs MSYS2 package `mingw-w64-clang-x86_64-libc++`, and the script can use `RUSTDESK_HARMONY_LIBCXX_INCLUDE`, `OHOS_LIBCXX_INCLUDE`, or the action-managed MSYS2 libc++ include root as fallback libc++ headers.
- Online run `27458205351` for commit `60e36f5` proved the package install step succeeded, but the build script still inspected a preinstalled `C:\msys64` instead of the action-managed setup-msys2 root. Fix: resolve `msys2.cmd` first, use `cygpath -w /usr/bin/bash.exe`, and discover `/clang64/include/c++/v1` through that bash instance. The workflow also writes `RUSTDESK_HARMONY_LIBCXX_INCLUDE` when it finds the include root.
- A local decoder-only libvpx attempt was rejected: it built `libvpx.a`, but Cargo failed because `scrap/src/bindings/vpx_ffi.h` requires `vpx/vp8cx.h` and `vpx/vpx_encoder.h`. Keep libvpx VP8/VP9 encoders enabled unless the `scrap` bindings and `common/vpxcodec.rs` are changed together.
- Final local validation after restoring full libvpx encoder support passed in `C:\rustdesk_harmony_decoder_validate`: `libvpx.a` (`3,302,224` bytes), full Cargo exit code `0`, local core size `129,592,014` bytes, SHA256 `32F3B3AC37EC82C94F2B4B3BA041459D2AF8ADA6AB1F3A57B39056F460F61B5F`.
