...
> 项目状态提示（2026-06-21 23:23）：本文仅是可选的主机资源优化附录，不属于 Core/HAP 构建、测试或发布步骤，不得为构建而自动关闭用户服务。当前权威构建与路径规则见 `CORE.md` 和 `WORKSPACE_PATHS.md`。

> 2026-06-22 最终发布已完成：Core `a7f7795` / `core-34` 与线上 HAP 双设备复验结果见 `CORE.md`；本附录不改变已冻结的构建测试规范。
